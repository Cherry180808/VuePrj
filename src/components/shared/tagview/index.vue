...
export default {
  ...
  methods: {
    ...
    moveToCurrentTag() {
      ...
      const doms = document.getElementsByClassName("tags-view-item");

      this.$nextTick(() => {
        for (const tag of doms) {
          this.$refs.scrollPane.moveToTarget(tag);
          // when query is different then update
          if (tag.pathname !== this.$route.fullPath) {
            this.$store.dispatch("tagView/updateVisitedView", this.$route);
          }
          break;
        }
    ...
    refreshSelectedTag(view) {
      this.$store.dispatch("tagView/delCachedView", view).then(() => {
        const { fullPath } = view;
        this.$nextTick(() => {
          this.$router.replace({
    ...
    closeSelectedTag(view) {
      this.$store.dispatch("tagView/delView", view).then(({ visitedViews }) => {
    ...
    closeOthersTags() {
      ...
      this.$store
        .dispatch("tagView/delOthersViews", this.selectedTag)
        .then(() => {
          this.moveToCurrentTag();
        });
    },
